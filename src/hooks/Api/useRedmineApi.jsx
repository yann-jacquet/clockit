...
const useRedmineApi = () => {
  ...
  const testFirstConnection = (apiUrl, apiKey, job) =>
    request("GET", `${apiUrl}/issues.json?assigned_to_id=me`, null, {
  ...
  const getIssuesAssignedTo = (userId) =>
    request("GET", `/issues.json?assigned_to_id=${userId || "me"}`);

  const getTask = (taskId) =>
    request("GET", `${apiParams.apiUrl}/issues.json?issue_id=${taskId}`, null, {
      axiosOptions: { headers: { "X-Redmine-API-Key": apiParams.apiKey } },
    });

  const getTimeEntry = (timeEntryId) =>
    request(
      "GET",
      `${apiParams.apiUrl}/time_entries/${timeEntryId}.json`,
      null,
      {
        axiosOptions: { headers: { "X-Redmine-API-Key": apiParams.apiKey } },
      }
  ...
  const postTimeEntry = (task) => {
    const xmlBodyStr = `<time_entry>
              ...
              <spent_on>${format(
                task.timeTracking.startTimestamp,
                "yyyy-MM-dd"
              )}</spent_on>
              <hours>${msToHours(
                task.timeTracking.endTimestamp -
                  task.timeTracking.startTimestamp
              )}</hours>
              <activity_id>9</activity_id>
              <custom_fields type="array">
                <custom_field id="41"><value>${
                  apiParams.job
                }</value></custom_field>
              </custom_fields>
              <comments></comments>
    ...
    return request("POST", `${apiParams.apiUrl}/time_entries.xml`, xmlBodyStr, {
      axiosOptions: {
        headers: {
